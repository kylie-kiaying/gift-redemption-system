...
export class SQLiteDB {
  ...
  public showTable(): void {
    ...
    this.db.all(sql, [], (err, rows: RedemptionRecord[]) => {
      ...
      rows.forEach((row) => {
        console.log(
          `${row.id}: Team ${row.team_name} redeemed at ${new Date(
            row.redeemed_at
          ).toLocaleString()}`
        );
      });
  ...
  // Clear all records from the redemption_records table
  public clearRedemptionRecords(): void {
    const sql = `DELETE FROM redemption_records`;

    this.db.run(sql, (err) => {
      if (err) {
        console.error("Error clearing redemption records", err.message);
      } else {
        console.log("All redemption records have been cleared.");
      }
    });
  }
...
// Uncomment the following line to clear all records in the redemption_records table
// dbInstance.clearRedemptionRecords();